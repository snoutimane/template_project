...
export class AddCandidateComponent {
  ...
  validateForm():boolean{
    ...
    if (!this.newCandidate.proposedDate ||
        ...
        !this.newCandidate.projectManageremail ||
  ...
  addCandidate() {
    ...
    if (this.newCandidate.interviewed) {
      ...
      // Check if the interview date is less than current date
    ...
    if (!this.isValidManagerEmail(this.newCandidate.projectManageremail)) {
    ...
    this.httpClient.post('http://localhost:8089/interviews/create', this.newCandidate)
      ...
      alert('Candidate added successfully!');
        // Optionally, handle success and error responses here
      }, (error) => {
  ...
  private isValidManagerEmail(projectManageremail: string): boolean {
    const emailPattern = /^[a-zA-Z0-9._%+-]+@capgemini\.com$/;
    return emailPattern.test(projectManageremail);
  }
  ...
  base64code!: any;
  dataReadyForUpload: boolean = false; // Track whether data is ready for upload
  ...
  onFileChange(event: any) {
    ...
    reader.onload = (e: any) => {
      ...
      this.uploadedData = excelData.slice(1).map((row) => ({
        ...
        comment: row[11],
        feedbackAttachment: row[12],
        proposedDate: new Date(row[13]),
        mentorName: row[14],
      }));
